...
Every verdict in the payload is decided in Python before it is stored. The script sets a colour and swaps a sentence; it owns no threshold and no projection, so `state_for_uptime` never acquires a JavaScript twin that drifts from it. The one thing the browser must decide for itself is freshness, because only the browser knows what time it is when the page is read — a payload written at 09:00 and read at 14:00 still says "operational", and only the reader's clock catches that. So the gate travels with the payload as a number of seconds and the arithmetic on the far side is a subtraction.

**The script is an upgrade, never a requirement.** The document that arrives is already correct as of its build and renders completely without it. A reader with JavaScript off, whose fetch is blocked, or who loads the page while the endpoint is down sees the built-in state and its honest timestamp — which is why every failure path in the script leaves the document exactly as it arrived, and why it writes no error anywhere. Somebody reading a status page during an outage is the last person who should be shown a second failure.

A Pages Function rather than a Worker, and that is not a small distinction. A Worker needs either a Custom Domain, which requires the zone to be on Cloudflare and `bakobo.com`'s is at Namecheap, or a `*.workers.dev` hostname, which some corporate filters block wholesale — a bad property for a page whose readers are strangers at work. Served from the site's own origin there is no second hostname, no CORS, and nothing extra for a proxy to refuse.
...
A snapshot older than three probe periods stops being evidence. Past that the page shows today grey and names the time measurement stopped, rather than projecting forward from another hour. Grey and not red: a stopped harvest or an expired token has nothing to do with whether the estate is up, so red there would publish a monitoring failure as an outage. That the harvest stopped is worth an alarm to the operator, and it gets one from the `status-now` dead-man's switch — pinged by the cron host on every run, with a period matching its fifteen-minute timer, so two consecutive misses raise it. Not the deploy job's switch, which this sentence used to name and which watches something else entirely: whether a *content* publish succeeded. A harvest that stops writing KV produces no deploy at all, so that switch would have stayed perfectly green throughout.
